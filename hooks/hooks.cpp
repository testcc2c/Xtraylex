...
namespace hooks
{
	void setup_hooks()
	{
		...
		if (MH_CreateHook(gameventmanag_target, &shonax_hooks::FireEventClientSide, reinterpret_cast<void**>(&shonax_hooks::oFireEventClientSidefn)) != MH_OK) {
			MessageBoxA(NULL, "Outdated index - fire_event_hook", MB_OK, MB_ICONERROR);
		}

		if (MH_CreateHook(retrievemessage_target, &shonax_hooks::GCRetrieveMessageHook, reinterpret_cast<void**>(&shonax_hooks::oGCRetrieveMessage)) != MH_OK) {
			MessageBoxA(NULL, "Outdated index - retrieve_message", MB_OK, MB_ICONERROR);
		}

		if (MH_CreateHook(sendmessage_target, &shonax_hooks::GCSendMessageHook, reinterpret_cast<void**>(&shonax_hooks::oGCSendMessage)) != MH_OK) {
			MessageBoxA(NULL, "Outdated index - send_message", MB_OK, MB_ICONERROR);
		}
